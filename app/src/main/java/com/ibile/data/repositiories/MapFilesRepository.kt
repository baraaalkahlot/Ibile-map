...
class MapFilesRepository(
    ...
    private fun initializeMapFiles(jsonFile: File): List<MapFile> = if (jsonFile.createNewFile()) {
        ...
        mapsDataJsonFile.writer()
            .use { gson.toJson(listOf(defaultMap), MAP_FILES_TYPE_TOKEN, it) }
        listOf(defaultMap)
    } else {
    ...
    private fun updateMapFiles(updatedList: List<MapFile>) {
        mapsDataJsonFile.writer().use { gson.toJson(updatedList, MAP_FILES_TYPE_TOKEN, it) }
        val updatedResult = mapsDataJsonFile.reader().use { gson.fromJson<List<MapFile>>(it, MAP_FILES_TYPE_TOKEN) }
